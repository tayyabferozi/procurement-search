...
async function searchRecords(
  { buyerID, textSearch }: RecordSearchFilters,
  offset: number,
  limit: number
): Promise<ProcurementRecord[]> {
  if (textSearch) {
    // If text search and buyer filter, both are applied
    if (buyerID) {
    ...
    // If text search is applied but buyer filter is not applied
    ...
    // If text search is not applied but buyer filter is applied
    ...
    // If neither the text search nor the buyer filter is applied
